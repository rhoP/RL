...
class RLTrajectoryStorage:
    ...
    def add_trajectory(self, states, actions, rewards, dones):
        ...
        if self.action_dim is None and len(actions) > 0:
            self.action_dim = (
                actions[0].shape[0]
                if hasattr(actions[0], "shape") and not all(actions[0].shape)
                else len(actions[0])
            )
    ...
    def get_state_action_pairs(self, max_samples=10000):
        ...
        for traj in self.trajectories:
            ...
            for i in range(min_len):
                state = (
                    states[i].flatten() if hasattr(states[i], "flatten") else states[i]
                )
                action = (
                    actions[i].flatten()
                    if hasattr(actions[i], "flatten")
                    else actions[i]
                )
...
class TrajectoryCollectorCallback(BaseCallback):
    ...
    def _on_step(self):
        ...
        state = self.locals["new_obs"][0]  # Get first env state
...
class ManifoldTriangulation:
    ...
    def compute_triangulation(self, points=None):
        ...
        if points is None:
            if self.projected_points is None:
                raise ValueError(
                    "No points provided. Call fit_dimension_reduction first."
                )
    ...
    def get_simplices(self):
        ...
        if self.triangulation is None:
            raise ValueError(
                "No triangulation computed. Call compute_triangulation first."
            )
    ...
    def visualize_triangulation_2d(
        ...
        plt.title(
            f"Delaunay Triangulation of State-Action Manifold ({self.method.upper()})"
        )
    ...
    def analyze_manifold_properties(self):
        ...
        properties = {
            ...
            "avg_simplex_volume": np.mean(
                [self._simplex_volume(points[s]) for s in simplices]
            ),
    ...
    def _simplex_volume(self, simplex_points):
        ...
        if len(simplex_points) == 3:  # Triangle
            return 0.5 * np.abs(
                np.cross(
                    simplex_points[1] - simplex_points[0],
...
def train_ppo_and_collect_trajectories(env_name="Pendulum-v1", total_timesteps=100000):
...
def collect_additional_trajectories(model, storage, env_name, num_trajectories=50):
    ...
    while collected < num_trajectories:
        ...
        while True and inner < max_inner:
...
def demonstrate_with_lunar_lander():
    ...
    model, storage, env = train_ppo_and_collect_trajectories(
        env_name="LunarLander-v3", total_timesteps=100000
    )
...
def demonstrate_with_pendulum():
    ...
    model, storage, env = train_ppo_and_collect_trajectories(
        env_name="Pendulum-v1", total_timesteps=50000
    )
...
def run_exp():
    ...
    print(
        f"Explained variance ratio (PCA): {triangulator.dimension_reducer.explained_variance_ratio_}"
    )
    ...
    triangulator.visualize_triangulation_2d(
        state_action_pairs=state_action_pairs, color_by_reward=True, storage=storage
    )
...
class SubManifoldAnalyzer:
    ...
    def identify_submanifolds(self, state_action_pairs, method="dbscan", **kwargs):
        ...
        if method == "dbscan":
            clustering = DBSCAN(
                eps=kwargs.get("eps", 0.5), min_samples=kwargs.get("min_samples", 10)
            )
        elif method == "kmeans":
    ...
    def visualize_submanifolds(self, state_action_pairs=None):
        ...
        if self.projected_points is None:
            raise ValueError(
                "No projected points available. Call identify_submanifolds first."
            )
    ...
    def extract_representative_trajectories(self, storage, n_trajectories=3):
        ...
        for label in self.submanifolds.keys():
            ...
            for traj_idx, traj in enumerate(storage.trajectories):
                ...
                for i in range(len(storage.trajectories)):
                    if i == traj_idx:
                        ...
                        break
                    current_idx += min(
                        len(storage.trajectories[i]["states"]),
            ...
            top_trajectories = sorted(
                traj_scores.items(), key=lambda x: x[1], reverse=True
            )[:n_trajectories]
...
def analyze_lunar_lander_submanifolds(storage):
    ...
    representative_trajs = analyzer.extract_representative_trajectories(
        storage, n_trajectories=2
    )
    ...
    for label, trajs in representative_trajs.items():
        ...
        for traj_idx, score in trajs:
            ...
            print(
                f"  Trajectory {traj_idx}: {score} points, total reward: {traj_reward:.2f}"
            )
...
def plot_submanifold_behavior_comparison(analyzer, characteristics):
    ...
    for label, chars in characteristics.items():
        axes[0, 0].bar(
            [f"M{label}-A{i}" for i in range(len(chars["mean_action"]))],
            chars["mean_action"],
            alpha=0.7,
            label=f"Submanifold {label}",
        )
    ...
    for i, (idx, name) in enumerate(zip(key_state_indices, key_state_names)):
        ...
        axes[0, 1].bar(
            [f"M{label}" for label in characteristics.keys()],
            values,
            alpha=0.7,
            label=name,
        )
    ...
    traj_counts = [
        len(chars["contributing_trajectories"]) for chars in characteristics.values()
    ]
    axes[1, 1].bar(
        [f"Submanifold {label}" for label in characteristics.keys()], traj_counts
    )
...
def run_anal(path=None):
    ...
    for label, mode_info in behavioral_modes.items():
        ...
        print(
            f"  Typical State: Altitude={mode_info['altitude']:.3f}, "
