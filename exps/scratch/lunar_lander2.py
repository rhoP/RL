...
def collect_successful_trajectories(model, env, num_episodes=1000, success_threshold=200):
    ...
    for episode in range(num_episodes):
        ...
        while not (terminated or truncated) and steps < 1000:
            # Ensure action is an integer, not a tuple
            action_result = model.predict(state, deterministic=True)
            action = action_result[0]  # Extract the integer action
            if isinstance(action, (tuple, list, np.ndarray)):
                action = action[0]  # Take first element if it's a container
...
def create_2d_trajectory_plot(state_action_sequences):
    ...
    for trajectory in state_action_sequences:
        ...
        for sa in trajectory:
            ...
            all_y_positions.append(sa[0][1])  # y position
    ...
    for i, trajectory in enumerate(state_action_sequences):
        if i % 10 == 0:
            ...
            ang_velocities = [state[5] for state in states]
...
def create_action_timing_plot(state_action_sequences):
    ...
    for trajectory in state_action_sequences:
        ...
        traj_length = len(trajectory_actions)
        if traj_length >= 3:
            # Start phase (first third)
            start_actions = trajectory_actions[:traj_length//3]
            phase_actions[0].extend(start_actions)
            
            # Middle phase
            middle_actions = trajectory_actions[traj_length//3:2*traj_length//3]
            phase_actions[1].extend(middle_actions)
            
            # End phase (last third)
            end_actions = trajectory_actions[2*traj_length//3:]
...
def analyze_trajectory_patterns(trajectories, state_action_sequences):
    ...
    final_positions = [traj[-1]['state'][:2] for traj in trajectories]
    final_velocities = [traj[-1]['state'][2:4] for traj in trajectories]
...
if __name__ == "__main__":
    ...
    model, env = train_lunar_lander_policy(total_timesteps=100000)
    
    # Collect successful trajectories
    trajectories, state_action_sequences = collect_successful_trajectories(
        model, env, num_episodes=100, success_threshold=100
    )
