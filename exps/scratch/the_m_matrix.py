...
class OptimalPPORolloutCollector:
    ...
    def train_optimal(self, total_timesteps=100000):
        ...
        mean_reward, std_reward = evaluate_policy(
            self.model, self.env, n_eval_episodes=100
        )
    ...
    def collect_rollouts_direct(self, num_rollouts=10):
        ...
        for rollout_idx in range(num_rollouts):
            ...
            print(
                f"Rollout {rollout_idx + 1}/{num_rollouts}: collected {len(episodes_from_rollout)} episodes"
            )
    ...
    def _collect_single_rollout(self):
        ...
        episodes = []
        current_episode = {"states": [], "actions": [], "rewards": [], "dones": []}
        ...
        while steps_collected < self.model.n_steps and not done:
            ...
            # Store step data
            current_state = (
                obs[0] if isinstance(obs, np.ndarray) and len(obs.shape) > 0 else obs
            )
            current_episode["states"].append(current_state)
            current_episode["actions"].append(
                actions[0] if isinstance(actions, np.ndarray) else actions
            )
            current_episode["rewards"].append(
                rewards[0] if isinstance(rewards, np.ndarray) else rewards
            )
            current_episode["dones"].append(
                dones[0] if isinstance(dones, np.ndarray) else dones
            )
...
def analyze_rollout_data(M, r, episode_data, blocks):
    ...
    print(
        f"Success rate: {success_rate:.3f} ({np.sum(r > 0)}/{len(r)} successful episodes)"
    )
    ...
    print(f"\nState coverage analysis:")
    print(
        f"States visited in at least one episode: {np.sum(state_coverage > 0)}/{M.shape[0]}"
    )
    print(
        f"Average state coverage per episode: {np.mean(np.sum(M, axis=0)):.1f} states"
    )
    ...
    if all_actions:
        ...
        for action, count in zip(unique_actions, action_counts):
            print(
                f"  {action_names[action]}: {count} times ({count / len(all_actions) * 100:.1f}%)"
            )
...
class FeatureBasedPPOCollector:
    ...
    def _create_feature_similarity_matrix(
        self, episode_features, similarity_threshold=0.7
    ):
...
def main():
    ...
    success_rate, avg_reward = analyze_rollout_data(
        M_triangular, r, episode_data, blocks
    )
...
class ContinuousStatePPOCollector:
    ...
    def fit_discretization(self, n_samples=10000):
        ...
        print(
            f"Fitted discretization with {self.n_state_bins} bins and {self.n_clusters} clusters"
        )
        return states
    ...
    def create_state_episode_matrices(self, episode_data):
        ...
        state_features = np.array(
            all_states
        )  # Shape: (total_steps_across_episodes, state_dim)
...
def continuous_space_workflow():
    ...
    collector = ContinuousStatePPOCollector(
        env_name="Pendulum-v1", n_state_bins=20, n_clusters=30
    )
    ...
    M_cluster_triangular, cluster_ordering = (
        collector.create_block_triangular_continuous(matrices, method="cluster")
    )
    ...
    print(
        f"Unique state clusters visited: {unique_clusters_visited}/{collector.n_clusters}"
    )
